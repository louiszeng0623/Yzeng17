...
若以上都为空，使用 Playwright 打开你给的页面，点击“赛程/赛程赛果”页签抓取表格。
"""
...
# Playwright 兜底（如果环境没装会在 workflow 装好；本地跑也可 pip install playwright 并 playwright install chromium）
try:
...
def headers_html():
    return {"User-Agent":"Mozilla/5.0 (iPhone; CPU iPhone OS 17_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.5 Mobile/15E148 Safari/604.1",
            "Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8","Referer":"https://www.dongqiudi.com/"}
...
def name_hit(name: str, aliases: List[str]) -> bool:
    n = norm(name); return any(a in n for a in [norm(x) for x in aliases])
...
def preserve_old_if_empty(path: str, new_rows: List[Dict]) -> bool:
    ...
    if os.path.exists(path) and os.path.getsize(path) > 0:
        print(f"🛟 新数据为空，保留旧文件：{path}"); return True
    return False

def append_report(lines: List[str]):
    os.makedirs("data", exist_ok=True)
    with open("data/parse_report.txt","w",encoding="utf-8") as f: f.write("\n".join(lines))
...
def deep_walk(obj: Any):
    if isinstance(obj, dict):
        ...
        if ("start_play" in keys or "match_time" in keys) and (
            ...
            yield obj
        for v in obj.values(): yield from deep_walk(v)
    elif isinstance(obj, list):
        for it in obj: yield from deep_walk(it)

def parse_dqd_html(html: str) -> List[Dict]:
    pats = [r"__NUXT__\s*=\s*(\{.*?\});", r"window\.__INITIAL_STATE__\s*=\s*(\{.*?\});", r"window\.__NUXT__\s*=\s*(\{.*?\});"]
    for pat in pats:
        ...
        if m:
            try:
                ...
                if found: print(f"🔎 DQD HTML 提取 {len(found)} 条"); return found
...
def api_pick_matches(payload: Any) -> List[Dict]:
    ...
    if isinstance(data, dict):
        for k in ("matches","list","schedules","items"):
            v = data.get(k); if isinstance(v, list): return v
    for k in ("matches","list","schedules","items"):
        v = payload.get(k); if isinstance(v, list): return v
    return []
...
def normalize_row(item: Dict, aliases: List[str]) -> Optional[Dict]:
    ...
    if isinstance(item.get("start_play"), (int,float)):
        ...
        if isinstance(mt, str):
            try:
                dt_try = datetime.fromisoformat(mt.replace("Z","+00:00")); ts = int(dt_try.timestamp())
            except Exception: pass
        elif isinstance(mt,(int,float)): ts = int(mt)
...
def looks_like_team(s: str) -> bool:
    s = s.strip(); return bool(re.search(r"[\u4e00-\u9fa5A-Za-z]", s)) and 1 <= len(s) <= 25
    ...
    for table in soup.find_all("table"):
        for tr in table.find_all("tr"):
            ...
            for i, c in enumerate(tds):
                if SCORE_RE.match(c): score_idx = i; break
            ...
            if name_hit(home, aliases): my_is_home, opponent = True, away
            elif name_hit(away, aliases): my_is_home, opponent = False, home
            else: my_is_home, opponent = None, away
...
def parse_zb8_text_grep(html: str, aliases: List[str]) -> List[Dict]:
    ...
    for m in re.finditer(r"(\d{4}-\d{1,2}-\d{1,2})", text):
        date = m.group(1); window = text[m.end():m.end()+180]
        t = re.search(r"(\d{1,2}:\d{2})", window); time_local = t.group(1) if t else "20:00"
        vs = re.search(r"\b(VS|vs)\b", window); sc = re.search(r"\b(\d+\s*[-:]\s*\d+)\b", window)
        ...
        home=away=None
        if vs: home, away = pick_team(window[:vs.start()]), pick_team(window[vs.end():])
        elif sc: home, away = pick_team(window[:sc.start()]), pick_team(window[sc.end():])
        if not (home and away): continue
        if name_hit(home, aliases): my_is_home, opponent = True, away
        elif name_hit(away, aliases): my_is_home, opponent = False, home
        else: my_is_home, opponent = None, away
        rows.append({"date":date,"time_local":time_local,"opponent":opponent,
                     "home_away":"Home" if my_is_home is True else ("Away" if my_is_home is False else "Unknown"),
                     "competition":"","stadium":"","status":""})
    print(f"🔎 ZB8 文本兜底解析 {len(rows)} 条")
    return rows

# ---- Playwright 兜底：点击“赛程/赛程赛果”页签抓表格 ----
    ...
    for table in soup.find_all("table"):
        for tr in table.find_all("tr"):
            ...
            if score_or_vs is not None:
                if score_or_vs-1 >= 0: home = cols[score_or_vs-1]
                if score_or_vs+1 < len(cols): away = cols[score_or_vs+1]
            if not (home and away): continue
            if name_hit(home, aliases): ha, opp = "Home", away
            elif name_hit(away, aliases): ha, opp = "Away", home
            else: ha, opp = "Unknown", away
            comp = ""
            for c in cols:
                if any(k in c for k in ("联","杯","甲","超","冠","欧","亚")): comp = c; break
            rows.append({"date":date,"time_local":time_local,"opponent":opp,"home_away":ha,"competition":comp,"stadium":"","status":""})
    return rows

def playwright_click_and_grab(url: str, tab_keywords: List[str], aliases: List[str]) -> List[Dict]:
    if not HAS_PW: return []
    try:
            ...
            if not clicked:
                ...
                for i in range(n):
                    ...
                    if any(k in tx for k in tab_keywords):
                        try:
                            els.nth(i).click(timeout=8000); clicked=True; break
                        except Exception:
                            continue
...
def fetch_team(team_key: str, info: Dict) -> Tuple[List[Dict], List[str]]:
    ...
    if info.get("zb8_page"):
        ...
        if zr and zr.status_code == 200 and zr.text:
            ...
            rows_from_zb8 = parse_zb8_table(zr.text, aliases) or parse_zb8_text_grep(zr.text, aliases)
